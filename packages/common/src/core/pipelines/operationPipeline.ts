...
> {
    ...
    public async execute(): Promise<OperationResult<Output> | Output> {
        try {
		    ...
		    if(!executionResult.success || executionResult.error) {
				// TODO: logger instead
				const error = executionResult.error ? this.mapToApplicationError(executionResult.error) : null
			...
			let currentResult: any = executionResult;

            for(const hook of this.postHooks) {
                currentResult = await hook(currentResult);
            }

            return currentResult as Output;
        } catch (error: any) {
            return {
                data: null,
