...
// TODO: work on adding manifests for these separately
const HelloworldComponentName = Object.freeze({
...
export const HELLOWORLD_COMPONENTS = Object.freeze([
	ComponentName.ALERT,
...
export const HELLOWORLD_COMPONENTS_BY_CATEGORY: ComponentsByCategoryType = Object.fromEntries(
    Object.entries(BASE_COMPONENTS_BY_CATEGORY).map(([category, components]) => [
        ...
        components.filter((component) =>
            (HELLOWORLD_COMPONENTS as readonly string[]).includes(component)
        ),
...
export const COMPONENT_REGISTRY = Object.fromEntries(
    Object.entries(BASE_COMPONENT_REGISTRY).filter(([key]) =>
        (HELLOWORLD_COMPONENTS as readonly string[]).includes(key)
    )
) as Pick<typeof BASE_COMPONENT_REGISTRY, typeof HELLOWORLD_COMPONENTS[number]>;
