...
import { PaginationActions, useToggle } from "@/hwiui/hooks";
import { CarouselControllerProps } from "@/hwiui/widgets/carousel";
...
function Controller({
    ...
    return (
        <div className="inline-flex justify-center">
            <IconButton disabled={!page || page.isFirst} icon={IconName.FAST_REWIND} onClick={handlePreviousClick} />
            {
                controllerState.showPause &&
                <IconButton icon={IconName.PAUSE_CIRCLE} onClick={handlePauseClick} />
            }
            {
                controllerState.showPlay &&
                <IconButton icon={IconName.PLAY_CIRCLE} onClick={handlePlayClick} />
            }
            {
                controllerState.showRestart &&
                <IconButton icon={IconName.RESTART_ALT} onClick={handleRestartClick} />
            }
            <IconButton disabled={!page || page.isLast} icon={IconName.FAST_FORWARD} onClick={handleNextClick} />
        </div>
    );
}

export default memo(displayName()(Controller));
