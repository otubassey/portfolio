...
import { ICON_NAMES } from "@/pages/ui/widgets/icon";
import IconButton from "@/pages/ui/widgets/icon/iconButton";
import { memo, Reducer, useCallback, useEffect, useReducer } from "react";
...
type OnControllerHandler = (event: HTMLButtonElement) => void;

type OnControllerChangeHandler = (event: HTMLButtonElement, status: ControllerOnChangeStatus) => void;
...
function Controller({active = false, onPrevious, onChange, onNext}: ControllerProps) {
    ...
    const handlePauseClick = useCallback(event => {
    ...
    const handleRestartClick = useCallback(event => {
